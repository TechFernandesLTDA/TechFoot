...
function makeClub([division, name, state], index) {
  ...
  const players = positions.map((position, playerIndex) => {
    ...
    return {
      ...
      nationality: "BRA",
      position,
      preferredPositions: position === "GK" ? ["GK"] : [position, position === "DF" ? "MF" : position === "FW" ? "MF" : "DF"],
      age: 18 + ((index * 3 + playerIndex * 7) % 18),
      xp: (index * 11 + playerIndex * 13) % 101,
      skills: {
        pace: Math.min(100, strength * 2 + ((index + playerIndex * 3) % 17)),
        finishing: position === "FW" ? Math.min(100, strength * 2 + 16 + playerIndex % 12) : Math.min(100, strength * 2 + playerIndex % 14),
        passing: position === "MF" ? Math.min(100, strength * 2 + 18 + index % 11) : Math.min(100, strength * 2 + 8 + playerIndex % 13),
        marking: position === "DF" ? Math.min(100, strength * 2 + 18 + playerIndex % 10) : Math.min(100, strength * 2 + playerIndex % 12),
        tackling: position === "DF" ? Math.min(100, strength * 2 + 15 + index % 13) : Math.min(100, strength * 2 + playerIndex % 15),
        handling: position === "GK" ? Math.min(100, strength * 2 + 22 + index % 9) : Math.min(100, strength * 2 + playerIndex % 11),
        stamina: Math.min(100, strength * 2 + 15 + (index + playerIndex) % 14),
        leadership: Math.min(100, strength * 2 + 5 + (index * 2 + playerIndex) % 22),
      },
      fitness: 78 + ((index + playerIndex * 5) % 23),
      morale: 65 + ((index + playerIndex) % 31),
      strength,
